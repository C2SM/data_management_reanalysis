...
def extract_vars_change_metadata(nc_file, var, era5_info):
    ...
    try:
        cmd = ['ncatted',
                '-a',
                f'standard_name,{var},c,c,{era5_info[var]['long_name']}',
                f'{work_out}/{var}_{basename}.nc',
    ...
    try:
        cmd = ['ncatted',
                '-a',
                f'units,{var},c,c,"{era5_info[var]['units']}"',
                f'{work_out}/{var}_{basename}_ncatted.nc',
    ...
    for year in range(startyr, endyr + 1):
        ...
        if store == 'cds':
            for month in months:
                ...
                nc_file = download_file_path.replace(suffix=".nc")
                try:
                    cdo.sorttaxis(option="-f nc4", input=download_file_multvar, output=nc_file)
                except Exception as e:
                ...
                nc_file_plus1 = download_file_plus1_path.replace(suffix=".nc")
                try:
                    cdo.sorttaxis(option="-f nc4", input=download_file_multvar_plus1, output=nc_file_plus1)
                except Exception as e:
