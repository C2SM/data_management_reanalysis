...
def download_data_cds_multvar(dataname, varlist, download_path, workdir, year, month, statistic="daily_mean"):
    target = f'{workdir}/variables_{statistic}_{dataname}_{year}{month}.nc'
    ...
    for year in range(startyr, endyr + 1):
        ...
        if store == 'cds':
            for month in months:
                ...
                download_file_multvar = download_data_cds_multvar(dataname, varlist_long, download_path, work_path, year, month, statistic)
                ...
                for v, var in enumerate(var_list_short):
                    ...
                    # 1. extract variables
                    #os.system(f'ncks -v {era5_info[var]["param"]} {download_file_multvar} {workdir}/{era5_info[var]["param"]}_{statistic}_era5-land_{year}{month}.nc')
                    file = f'{work_path}/{era5_info[var]["param"]}_{statistic}_{dataname}_{year}{month}.nc'
                    try:
                        cmd = [
                            'ncks',
                            '-v', era5_info[var]["param"],
                    ...
                    outfile = f'{proc_archive}/{era5_info[var]["cmip_name"]}_{statistic}_{dataname}_{year}{month}.nc'
                    if os.path.isfile(outfile) and not overwrite:
                        logger.info(f"File {outfile} already exists and overwrite is set to False. Skipping processing of variable {var} for month {month}.")
                        continue
