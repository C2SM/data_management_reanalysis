...
LOG_FILENAME = (f'logfiles/logging_process_ERA5_dkrz'
...
def convert_tcc(tcc_outfile, workdir, era5_info, year, month):
    ...
    os.system(f'cdo -b F64 mulc,100 {tcc_outfile} {workdir}/{era5_info["short_name"]}_era5_{year}{month}_mulc.nc')
    os.system(f'rm {workdir}/{era5_info["short_name"]}_era5_{year}{month}.nc')
    os.system(f'ncatted -a units,{era5_info["short_name"]},m,c,"{era5_info["cmip_unit"]}" {workdir}/{era5_info["short_name"]}_era5_{year}{month}_mulc.nc {tcc_outfile}')
...
def convert_tp(tp_outfile, workdir, era5_info, year, month):
    ...
    os.system(f'cdo -b F64 divc,86400 {tp_outfile} {workdir}/{era5_info["short_name"]}_era5_{year}{month}_divc.nc')
    os.system(f'rm {workdir}/{era5_info["short_name"]}_era5_{year}{month}.nc')
    os.system(f'ncatted -a units,{era5_info["short_name"]},m,c,"{era5_info["cmip_unit"]}" {workdir}/{era5_info["short_name"]}_era5_{year}{month}_divc.nc {tp_outfile}')
...
def convert_era5_to_cmip(tmp_outfile, outfile, config, era5_info, year, month):
    ...
    os.system(f'ncks -O -4 -D 4 --cnk_plc=g3d --cnk_dmn=time,1 --cnk_dmn=lat,{config.lat_chk} --cnk_dmn=lon,{config.lon_chk} {tmpfile}_remapped.nc {tmpfile}_chunked.nc')
...
def main():
    ...
    for v, var in enumerate(cfg.variables):
        ...
        for year in range(cfg.startyr, cfg.endyr+1):
            ...
            os.system(f'rsync -av levante:{dkrz_path}/E5sf{typeid}_{cfg.freq}_{year}-??_{vparam}.* {iac_path}')
            ...
            for month in  ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']:
                ...
                outfile_name = convert_era5_to_cmip(tmp_outfile, outfile, cfg, era5_info, year, month)
