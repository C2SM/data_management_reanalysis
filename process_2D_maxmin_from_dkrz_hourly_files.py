...
def calc_min(tmp_outfile, workdir, era5_info, year, month, day_str):
    ...
    os.system(f'cdo daymin {tmp_outfile} {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_min.nc')
    os.system(f'rm {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}.nc')
    os.system(f'cdo chname {era5_info["short_name"]},{era5_info["cmip_name"]}" {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_min.nc {tmp_outfile}')
...
def calc_max(tmp_outfile, workdir, era5_info, year, month, day_str):
    ...
    os.system(f'cdo daymax {tmp_outfile} {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_max.nc')
    os.system(f'rm {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}.nc')
    os.system(f'cdo chname {era5_info["short_name"]},{era5_info["cmip_name"]}" {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_max.nc {tmp_outfile}')
...
def main():
    ...
    for v, varout in enumerate(cfg.varout):
        ...
        for year in range(cfg.startyr, cfg.endyr+1):
            ...
            os.system(f'rsync -av levante:{dkrz_path}/E5sf00_{cfg.freq}_{year}-??-??_{vparam}.* {iac_path}')
