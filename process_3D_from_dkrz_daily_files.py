...
def convert_era5_to_cmip(
    ...
    os.system(
        f"ncks -O -4 -D 4 --cnk_plc=g3d --cnk_dmn=time,1 --cnk_dmn=level,{plev} --cnk_dmn=lat,{lat_chk} --cnk_dmn=lon,{lon_chk} {tmpfile}_remapped.nc {tmpfile}_chunked.nc"
    )
    os.system(
...
def main(cfg):
    ...
    if outfile.isfile() and not overwrite:
        ...
        dkrz_path = f"/pool/data/ERA5/E5/pl/{type}/1D/{vparam}"
        ...
        os.system(
            f"rsync -av levante:{dkrz_path}/E5pl{typeid}_{cfg.freq}_{year}-{month}_{vparam}.* {iac_path}"
        )
        ...
        proc_mon_archive = (
            f'{end_dir}/{era5_info["cmip_name"]}/mon/native/{year}'
        )
