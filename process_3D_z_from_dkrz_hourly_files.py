...
def convert_cc(tmp_outfile, workdir, era5_info, year, month, day_str):
    ...
    os.system(f'cdo mulc,100 {tmp_outfile} {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_mulc.nc')
    os.system(f'rm {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}.nc')
    os.system(f'ncatted -a units,{era5_info["short_name"]},m,c,"{era5_info["cmip_unit"]}" {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_mulc.nc {tmp_outfile}')
...
def convert_z(tmp_outfile, workdir, era5_info, year, month, day_str):
    ...
    const = 9.80665 
    os.system(f'cdo divc,{const} {tmp_outfile} {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_divc.nc')
    os.system(f'rm {tmp_outfile}')
    os.system(f'ncatted -a units,{era5_info["short_name"]},m,c,"{era5_info["cmip_unit"]}" {workdir}/{era5_info["short_name"]}_era5_{year}{month}{day_str}_divc.nc {tmp_outfile}')
...
def main():
    ...
    for v, var in enumerate(cfg.variables):
        ...
        for year in range(cfg.startyr, cfg.endyr+1):
            ...
            os.system(f'rsync -av levante:{dkrz_path}/E5pl00_{cfg.freq}_{year}-??-??_{vparam}.* {iac_path}')
            ...
            for month in  ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']:
                ...
                for day in days:
                    ...
                    tmp_outfile = convert_netcdf_add_era5_info(grib_file, cfg.work_path, era5_info, year, month, day_str)
                    ...
                    if (era5_info["unit"] != era5_info["cmip_unit"]):
                        if var == "cc":
                            ...
                            tmp_outfile = convert_cc(tmp_outfile, cfg.work_path, era5_info, year, month, day_str)
                        elif var == "z":
                            logger.info(f'Unit for z needs to be changed from {era5_info["unit"]} to {era5_info["cmip_unit"]}.')
                            tmp_outfile = convert_z(tmp_outfile, cfg.work_path, era5_info, year, month, day_str)
                        else:
                            logger.error(f'Conversion of unit for variable {var} is not implemented!')
                            sys.exit(1)

                    # calculate daily means
                    os.system(f'cdo daymean {tmp_outfile}  {cfg.work_path}/{var}_daymean_era5_{year}{month}{day_str}.nc')

                    if not os.path.isfile(f'{cfg.work_path}/{var}_daymean_era5_{year}{month}{day_str}.nc'):
                        logger.warning(f'{cfg.work_path}/{var}_daymean_era5_{year}{month}{day_str}.nc was not processed properly!')
                    else:
                        # clean up 1-hr data
                        os.system(f'rm {tmp_outfile}')
                ...
                os.system(f'cdo mergetime {cfg.work_path}/{var}_daymean_era5_{year}{month}*.nc {daily_file}')

                outfile_name = convert_era5_to_cmip(daily_file, outfile, cfg, era5_info, cmip_info, year, month)
