...
#variable_list=(tp strd ssrd str ssr)
variable_list=(tp)
...
variable_list=()
for var in "${variable_list[@]}"; do
    #python process_3D_from_dkrz_daily_files.py -c configs/Config_era5_1day_3D_dkrz.yaml -v "$var"
    echo "no processing"
done
...
variable_list=(z)
for var in "${variable_list[@]}"; do
    #python process_3D_z_from_dkrz_hourly_files.py -c configs/Config_era5_1hr_3D_dkrz.yaml -v "$var"
    echo "no processing"
done
